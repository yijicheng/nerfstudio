...
def interpolate_ms_features(
    ...
    for grid in grid_encodings:
        ...
        if concat_features:
            multi_scale_interp.append(grid_features) # type: ignore
        else:
...
class KPlanesField(Field):
    ...
    def get_density(self, ray_samples: RaySamples) -> Tuple[TensorType, TensorType]:
        ...
        if self.spatial_distortion is not None:
            ...
            # From [0, 1] to [-1, 1]
            positions = SceneBox.get_normalized_positions(positions, self.aabb) * 2.0 - 1.0 # type: ignore
        ...
        features = interpolate_ms_features(
            positions_flat, grid_encodings=self.grids, concat_features=self.concat_across_scales # type: ignore
        )
        if len(features) < 1:
        ...
        return density, features # type: ignore
    ...
    def get_outputs(
        ...
        if self.appearance_embedding_dim > 0:
            ...
            if not self.linear_decoder:
                color_features.append(embedded_appearance) # type: ignore

        color_features = torch.cat(color_features, dim=-1) # type: ignore
        if self.linear_decoder:
            basis_input = directions
            if self.appearance_ambedding_dim > 0: # type: ignore
                basis_input = torch.cat([directions, embedded_appearance], dim=-1) # type: ignore
        ...
        return {FieldHeadNames.RGB: rgb} # type: ignore
...
class KPlanesDensityField(Field):
    ...
    def density_fn(self, positions: TensorType["bs":..., 3], times: Optional[TensorType["bs", 1]] = None) -> TensorType["bs":..., 1]:
        ...
        if times is not None and (len(positions.shape) == 3 and len(times.shape) == 2):
            ...
            times = times[:, None]  # RaySamples can handle the shape # type: ignore
    ...
    def get_density(self, ray_samples: RaySamples) -> Tuple[TensorType, None]:
        ...
        density_before_activation = self.sigma_net(features).view(*ray_samples.frustums.shape, -1)
        density = trunc_exp(density_before_activation.to(positions) - 1)
        return density, None # type: ignore
