...
class KPlanesModelConfig(ModelConfig):
    ...
    num_proposal_samples: Optional[Tuple[int]] = (256, 128) # type: ignore
...
class KPlanesModel(Model):
    ...
    def populate_modules(self):
        ...
        self.proposal_sampler = ProposalNetworkSampler(
            num_nerf_samples_per_ray=self.config.num_samples, # type: ignore
            num_proposal_samples_per_ray=self.config.num_proposal_samples, # type: ignore
    ...
    def get_metrics_dict(self, outputs, batch):
        ...
        if self.training:
            ...
            prop_grids = [p.grids.plane_coefs for p in self.proposal_networks] # type: ignore
            field_grids = [g.plane_coefs for g in self.field.grids]

            metrics_dict["plane_tv"] = space_tv_loss(field_grids) # type: ignore
            metrics_dict["plane_tv_proposal_net"] = space_tv_loss(prop_grids) # type: ignore

            if len(self.config.grid_base_resolution) == 4:
                metrics_dict["l1_time_planes"] = l1_time_planes(field_grids) # type: ignore
                metrics_dict["l1_time_planes_proposal_net"] = l1_time_planes(prop_grids) # type: ignore
                metrics_dict["time_smoothness"] = time_smoothness(field_grids) # type: ignore
                metrics_dict["time_smoothness_proposal_net"] = time_smoothness(prop_grids) # type: ignore
